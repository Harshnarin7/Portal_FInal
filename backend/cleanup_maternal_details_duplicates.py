"""
One-time cleanup: merge duplicate maternal_details rows per enrollment_id.

Context
-------
POST /maternal-details/ used to always INSERT with no check for an existing
row. enrollment_id is not unique at the DB level, so a network retry or a
stale Form C "already exists" check could create a second row for the same
patient. GET/POST/PUT all used .first() with no ORDER BY, so the wrong copy
could load or update depending on undefined row order.
...
Usage (from the backend folder, with DATABASE_URL pointing at the target):
    python cleanup_maternal_details_duplicates.py

A review log is printed to stdout and written next to this script as
cleanup_maternal_details_duplicates.log so Harsh can inspect what changed.
"""
...
LOG_PATH = Path(__file__).resolve().parent / "cleanup_maternal_details_duplicates.log"
...
def main():
    ...
    try:
        dup_ids = (
            db.query(MaternalDetails.enrollment_id)
            .group_by(MaternalDetails.enrollment_id)
            .having(func.count(MaternalDetails.id) > 1)
            .all()
        )
        enrollment_ids = [eid for (eid,) in dup_ids if eid]

        log(f"maternal_details duplicate cleanup started {datetime.now().isoformat()}")
        ...
        for enrollment_id in sorted(enrollment_ids):
            ...
            db.flush()
            db.query(MaternalDetails).filter(MaternalDetails.id.in_(older_ids)).delete(
