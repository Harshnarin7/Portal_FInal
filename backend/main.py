...
# ============================================================================
# GA CHECK LOG — near-zero-friction live pre-screening capture (see ga_check.py)
# ============================================================================
...
@app.get("/ga-check/", response_model=List[GACheckEntryOut])
def list_ga_check_entries(
    site: str | None = None,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    ...
    if not is_global(current_user):
        query = query.filter(GACheckEntry.site_name == current_user.site_name)
    elif site:
        query = query.filter(GACheckEntry.site_name == site)
    records = query.order_by(GACheckEntry.check_date.desc().nulls_last(), GACheckEntry.id.desc()).all()
...
@app.patch("/ga-check/{entry_id}/link", response_model=GACheckEntryOut)
def link_ga_check_entry(
    entry_id: int,
    data: GACheckLinkRequest,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    """Called once the Form A screening the nurse continued into from a
    GA Check entry is actually saved -- closes the Box1(GA check)->Box5
    (eligible/screened) loop from this end, rather than relying on
    birth_log_matching's after-the-fact name/UID match (that one only
    fires at birth, this fires the moment the screening itself exists)."""
    ...
    record.continued_to_screening = True
    record.screening_id = data.screening_id
    db.commit()
    db.refresh(record)
    return record


@app.get("/ga-check/gap")
def get_ga_check_gap(
    site: str | None = None,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    """Eligible (GA<32wk) GA Check entries never continued into a saved
    Form A -- the live-side completeness gap, catching a miss at triage
    instead of waiting for it to surface at birth in the Log of All Births."""
    query = db.query(GACheckEntry).filter(
        GACheckEntry.eligible == True,  # noqa: E712
        GACheckEntry.continued_to_screening == False,  # noqa: E712
    )
    if not is_global(current_user):
        query = query.filter(GACheckEntry.site_name == current_user.site_name)
    elif site:
        query = query.filter(GACheckEntry.site_name == site)
    records = query.order_by(GACheckEntry.check_date.desc().nulls_last()).all()
