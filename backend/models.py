...
class Screening(Base):
    ...
    # Base64-encoded PNG data URL captured from the on-screen signature pad
...
class MaternalDetails(Base):
    ...
    steroid_beta_lddi_known = Column(String)
    steroid_beta_lddi_hours = Column(String)
    steroid_dexa_lddi_known = Column(String)
    steroid_dexa_lddi_hours = Column(String)
...
class NeonatalMorbidities(Base):
    ...
    # ================================================================
    # EXTENDED COLUMNS (added to match the full Form H / CRF field set)
    # ================================================================

    # ---------------- NEUROLOGICAL (H1) — extended ----------------
    aed_number = Column(String)
    aed_other = Column(String)
    aed_type = Column(JSON)
    ahw = Column(Float)
    eeg_result = Column(String)
    etiology = Column(String)
    etiology_other = Column(String)
    ...
    ivh_grade_left = Column(String)
    ivh_grade_right = Column(String)
    ivh_present = Column(String)
    pvl_date_left = Column(Date)
    pvl_date_right = Column(Date)
    ...
    age_steroid = Column(Integer)
    apnea_onset_age = Column(Integer)
    bpd_support_36w = Column(String)
    caffeine_duration = Column(Integer)
    caffeine_used = Column(String)
    cpap = Column(String)
    cpap_used = Column(String)
    extubation_episodes = Column(Integer)
    extubation_failure = Column(String)
    hc_after_first = Column(Boolean)
    hc_after_second = Column(Boolean)
    hc_first_drug = Column(Boolean)
    hfnc = Column(String)
    hfnc_days = Column(Integer)
    hfnc_used = Column(String)
    hydrocortisone_bp = Column(String)
    imv_days = Column(Integer)
    imv_used = Column(String)
    invasive_ventilation = Column(String)
    nasal_cannula = Column(String)
    nasal_cannula_days = Column(Integer)
    nasal_cannula_used = Column(String)
    nippv = Column(String)
    nippv_days = Column(Integer)
    nippv_used = Column(String)
    o2_days = Column(Integer)
    oxygen_exposure = Column(Float)
    pulmonary_hypertension = Column(String)
    rx_ino = Column(Boolean)
    rx_miliri = Column(Boolean)
    rx_other = Column(Boolean)
    rx_other_text = Column(String)
    ...
    age_first_feed = Column(Integer)
    age_full_feeds = Column(Integer)
    age_full_feeds_summary = Column(Integer)
    bifidobacterium = Column(String)
    ebm_days = Column(Integer)
    fi_abdominal_distension = Column(Boolean)
    ...
    nec_resection = Column(String)
    nec_resection_length = Column(Float)
    nec_stoma = Column(String)
    nec_surgery_type = Column(String)
    pdhm_days = Column(Integer)
    pn_acidosis = Column(Boolean)
    pn_adverse = Column(String)
    pn_cholestasis = Column(Boolean)
    pn_days_summary = Column(Integer)
    pn_electrolyte = Column(Boolean)
    pn_hypercapnia = Column(Boolean)
    pn_other = Column(Boolean)
    pn_other_text = Column(String)
    probiotic = Column(String)
    strain_bi = Column(Boolean)
    strain_mono = Column(Boolean)
    strain_multi = Column(Boolean)
    tpn_associated = Column(String)

    # ---------------- METABOLIC (H4) — extended ----------------
    alp_peak = Column(Float)
    dyselectro_ca = Column(Boolean)
    dyselectro_k = Column(Boolean)
    dyselectro_na = Column(Boolean)
    dyselectrolytemia = Column(String)
    hyperglycemia = Column(String)
    hyperglycemia_highest = Column(Float)
    hyperglycemia_rx = Column(String)
    hypoglycemia = Column(String)
    hypoglycemia_episodes = Column(Integer)
    hypoglycemia_lowest = Column(Float)
    hypoglycemia_rx = Column(String)
    hypoglycemia_rx_duration = Column(Integer)
    lowest_calcium = Column(Float)
    lowest_phosphorus = Column(Float)
    metabolic_acidosis = Column(String)
    osteopenia = Column(String)
    # H4.1 fields 106-111: Hypo/Hyper-natremia/kalemia/calcemia detail,
    # gated in the UI behind the H4.1 #105 Type checkboxes (dyselectro_na/k/ca)
    hyponatremia = Column(Boolean)
    hyponatremia_status = Column(String)
    hyponatremia_symptoms = Column(String)
    hypernatremia = Column(Boolean)
    hypernatremia_status = Column(String)
    hypernatremia_symptoms = Column(String)
    hypokalemia = Column(Boolean)
    hypokalemia_status = Column(String)
    hypokalemia_symptoms = Column(String)
    hyperkalemia = Column(Boolean)
    hyperkalemia_status = Column(String)
    hyperkalemia_symptoms = Column(String)
    hypocalcemia = Column(Boolean)
    hypocalcemia_status = Column(String)
    hypocalcemia_symptoms = Column(String)
    hypercalcemia = Column(Boolean)
    hypercalcemia_status = Column(String)
    hypercalcemia_symptoms = Column(String)

    # ---------------- CARDIOVASCULAR (H5) — extended ----------------
    dbp = Column(Float)
    fluid_bolus = Column(String)
    fluid_bolus_number = Column(Integer)
    hypotension_both = Column(Boolean)
    hypotension_diastolic = Column(Boolean)
    hypotension_systolic = Column(Boolean)
    inotrope_adr = Column(Boolean)
    inotrope_dobu = Column(Boolean)
    inotrope_dopa = Column(Boolean)
    inotrope_duration = Column(Integer)
    inotrope_milri = Column(Boolean)
    inotrope_nadr = Column(Boolean)
    inotrope_vaso = Column(Boolean)
    pda_both = Column(Boolean)
    ...
    pda_medical_rx = Column(String)
    pda_murmur = Column(Boolean)
    pda_other_feature = Column(Boolean)
    pda_other_feature_text = Column(String)
    pda_pattern_growing = Column(Boolean)
    pda_pattern_none = Column(Boolean)
    pda_pattern_pulsatile = Column(Boolean)
    pda_pcm = Column(Boolean)
    pda_peak_velocity = Column(Float)
    pda_shunt = Column(String)
    pda_systemic_steal = Column(String)
    pda_tdd = Column(Float)
    pda_wide_pp = Column(Boolean)
    sbp = Column(Float)
    structural_heart_disease = Column(String)
    structural_heart_disease_detail = Column(String)
    vis_score = Column(Float)

    # ---------------- HEMATOLOGY (H6) — extended ----------------
    anemia = Column(String)
    anemia_chf = Column(String)
    ...
    dvet = Column(String)
    dvet_number = Column(Integer)
    ffp_cryo = Column(String)
    ffp_number = Column(Integer)
    irradiated = Column(String)
    ivig = Column(String)
    ...
    platelet_number = Column(Integer)
    platelets = Column(String)
    prbc = Column(String)
    prbc_number = Column(Integer)
    prbc_volume = Column(Float)

    # ---------------- RENAL (H7) — extended ----------------
    aki = Column(String)
    aki_date = Column(Date)
    aki_dialysis = Column(String)
    aki_oliguria = Column(String)
    aki_peak_creatinine = Column(Float)
    aki_stage1 = Column(Boolean)
    ...
    rop_bilateral = Column(String)
    rop_comment = Column(String)
    ...
    rop_stage4 = Column(Boolean)
    rop_stage5 = Column(Boolean)
    rop_treatment = Column(String)
    rop_vitrectomy = Column(Boolean)
    ...
    hyperthermia_location_transport = Column(Boolean)
    ...
    hypothermia_environment = Column(Boolean)
    hypothermia_immaturity = Column(Boolean)
    hypothermia_ivh = Column(Boolean)
    hypothermia_location_dr = Column(Boolean)
    hypothermia_location_nicu = Column(Boolean)
    hypothermia_location_transport = Column(Boolean)
    hypothermia_lowest_temp = Column(Float)
    hypothermia_mild = Column(Boolean)
    hypothermia_moderate = Column(Boolean)
    hypothermia_other = Column(Boolean)
    hypothermia_other_text = Column(String)
    hypothermia_sepsis = Column(Boolean)
    hypothermia_severe = Column(Boolean)

    # ---------------- VASCULAR ACCESS (H9) — extended ----------------
    arterial_posterior_tibial = Column(Boolean)
    arterial_radial = Column(Boolean)
    extravasation = Column(String)
    ...
    picc = Column(String)
    picc_days = Column(Integer)
    uac = Column(String)
    uac_days = Column(Integer)
    uvc = Column(String)
    uvc_days = Column(Integer)

    # ---------------- INFECTION (H10) — cumulative totals ----------------
    vap_episodes = Column(Integer)

    # ---------------- HOSPITAL COURSE (H12) — extended ----------------
    back_referral_hospital = Column(String)
    back_referral_other = Column(String)
    designation = Column(String)
    discharge_hc = Column(Float)
    total_los = Column(Integer)
